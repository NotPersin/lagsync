...
#if !defined(IMGUI_DISABLE_INCLUDE_IM_config_H) || defined(IMGUI_INCLUDE_IM_config_H)
#include "imconfig.h"       // User-editable _configuration file
#endif
...
struct ImFont_config;                // _configuration data when adding a font or merging fonts
struct ImColor;                     // Helper functions to create a color that can be converted to either u32 or float4
struct ImGuiIO;                     // Main _configuration and I/O between your application and ImGui
...
#ifdef IM_VEC2_CLASS_EXTRA          // Define constructor and implicit cast operators in im_config->h to convert back<>forth from your math types and ImVec2.
	IM_VEC2_CLASS_EXTRA
#endif
};
...
#ifdef IM_VEC4_CLASS_EXTRA          // Define constructor and implicit cast operators in im_config->h to convert back<>forth from your math types and ImVec4.
	IM_VEC4_CLASS_EXTRA
#endif
};
...
namespace ImGui
{
	...
	IMGUI_API void          Tab_Button(const char* label, int& enabled, int best = 0, const ImVec2& size = ImVec2(0, 0));            // button
	IMGUI_API bool			Tab(const char* label, bool toggled, const ImVec2& size_arg = ImVec2(0, 0));
	IMGUI_API void			Switch(const char* label, const char* const* items, const ImVec2& size_arg, int array_size, int& cfg);
	...
	IMGUI_API bool          Checkbox(const char* label, bool* v, int colorpicker = false);
	IMGUI_API bool          CheckboxFlags(const char* label, unsigned int* flags, unsigned int flags_value);
	IMGUI_API bool          RadioButton(const char* label, bool active);
	IMGUI_API bool          RadioButton(const char* label, int* v, int v_button);
	IMGUI_API bool          Combo(const char* label, int* current_item, const char* const* items, int items_count, int height_in_items = -1, int color = 0);
	IMGUI_API bool          Combo(const char* label, int* current_item, const char* items_separated_by_zeros, int height_in_items = -1, int color = 0);    // separate items with \0, end item-list with \0\0
	IMGUI_API bool          Combo(const char* label, int* current_item, bool(*items_getter)(void* data, int idx, const char** out_text), void* data, int items_count, int height_in_items = -1, int color = 0);
	...
	IMGUI_API bool MultiCombo(const char* label, bool* current_item, bool(*items_getter)(void*, bool, const char**), void* data, int items_count, int height_in_items, int color);

	IMGUI_API bool MultiCombo(const char* label, bool** current_item, const char* const* items, int items_count, int height_in_items, int color);
...
enum ImGuiColorEditFlags2_ {
	ImGuiColorEditFlags_Alpha2 = 1 << 0,   // ColorEdit/ColorPicker: show/edit Alpha component. Must be 0x01 for compatibility with old API taking bool
	ImGuiColorEditFlags_RGB2 = 1 << 1,   // ColorEdit: Choose one among RGB/HSV/HEX. User can still use the options menu to change. ColorPicker: Choose any combination or RGB/HSX/HEX.
	ImGuiColorEditFlags_HSV2 = 1 << 2,
	ImGuiColorEditFlags_HEX2 = 1 << 3,
	ImGuiColorEditFlags_NoPicker2 = 1 << 4,   // ColorEdit: Disable picker when clicking on colored square
	ImGuiColorEditFlags_NoOptions2 = 1 << 5,   // ColorEdit: Disable toggling options menu when right-clicking colored square
	ImGuiColorEditFlags_NoColorSquare2 = 1 << 6,   // ColorEdit: Disable colored square
	ImGuiColorEditFlags_NoSliders2 = 1 << 7,   // ColorPicker: Disable RGB/HSV/HEX sliders
	ImGuiColorEditFlags_ModeMask_2 = ImGuiColorEditFlags_RGB2 | ImGuiColorEditFlags_HSV2 | ImGuiColorEditFlags_HEX2
};
...
struct ImGuiStyle
{
	...
	ImVec2		ColorOverride;				// Overrides the current pos of colors pickers since it resets current cursor postioning
	ImVec2		ComboOverride;				// Overrides the current pos of combo boxs, gives more freedom to moving it
	ImVec2		KeyBindOverride;			// Overrides the current pos of keybinds, gives more freedom to moving it
...
// Vertex index (override with '#define ImDrawIdx unsigned int' inside in im_config->h)
#ifndef ImDrawIdx
...
#else
// You can override the vertex format layout by defining IMGUI_OVERRIDE_DRAWVERT_STRUCT_LAYOUT in im_config->h
...
struct ImFontAtlas
{
	...
	IMGUI_API ~ImFontAtlas();
	IMGUI_API ImFont* AddFont(const ImFont_config* font_cfg);
	IMGUI_API ImFont* AddFontDefault(const ImFont_config* font_cfg = NULL);
	IMGUI_API ImFont* AddFontFromFileTTF(const char* filename, float size_pixels, const ImFont_config* font_cfg = NULL, const ImWchar* glyph_ranges = NULL);
	IMGUI_API ImFont* AddFontFromMemoryTTF(void* font_data, int font_size, float size_pixels, const ImFont_config* font_cfg = NULL, const ImWchar* glyph_ranges = NULL);                                       // Transfer ownership of 'ttf_data' to ImFontAtlas, will be deleted after Build()
	IMGUI_API ImFont* AddFontFromMemoryCompressedTTF(const void* compressed_font_data, int compressed_font_size, float size_pixels, const ImFont_config* font_cfg = NULL, const ImWchar* glyph_ranges = NULL); // 'compressed_font_data' still owned by caller. Compress with binary_to_compressed_c.cpp
	IMGUI_API ImFont* AddFontFromMemoryCompressedBase85TTF(const char* compressed_font_data_base85, float size_pixels, const ImFont_config* font_cfg = NULL, const ImWchar* glyph_ranges = NULL);              // 'compressed_font_data_base85' still owned by caller. Compress with binary_to_compressed_c.cpp with -base85 paramaeter
	...
	ImVector<ImFont_config>      _configData;         // Internal data
...
struct ImFont
{
	...
	short                       _configDataCount;    // ~ 1          // Number of ImFont_config involved in creating this font. Bigger than 1 when merging multiple font sources into one ImFont.
	ImFont_config* _configData;         //              // Pointer within ContainerAtlas->_configData
