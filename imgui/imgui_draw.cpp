...
void ImDrawList::AddRect(const ImVec2& a, const ImVec2& b, ImU32 col, float rounding, int rounding_corners_flags, float thickness)
{
	if ((col & IM_COL32_A_MASK) == 0)
		return;
	PathRect(a + ImVec2(0.50f, 0.50f), b - ImVec2(0.49f, 0.49f), rounding, rounding_corners_flags);
...
{
	for (int i = 0; i < _configData.Size; i++)
		if (_configData[i].FontData && _configData[i].FontDataOwnedByAtlas)
		{
			ImGui::MemFree(_configData[i].FontData);
			_configData[i].FontData = NULL;
		}

	// When clearing this we lose access to the font name and other information used to build the font.
	for (int i = 0; i < Fonts.Size; i++)
		if (Fonts[i]->_configData >= _configData.Data && Fonts[i]->_configData < _configData.Data + _configData.Size)
		{
			Fonts[i]->_configData = NULL;
			Fonts[i]->_configDataCount = 0;
		}
	_configData.clear();
	CustomRects.clear();
}
...
ImFont* ImFontAtlas::AddFont(const ImFont_config* font_cfg)
{
	...
	_configData.push_back(*font_cfg);
	ImFont_config& new_font_cfg = _configData.back();
...
ImFont* ImFontAtlas::AddFontDefault(const ImFont_config* font_cfg_template)
{
	ImFont_config font_cfg = font_cfg_template ? *font_cfg_template : ImFont_config();
...
ImFont* ImFontAtlas::AddFontFromFileTTF(const char* filename, float size_pixels, const ImFont_config* font_cfg_template, const ImWchar* glyph_ranges)
{
	int data_size = 0;
...
ImFont* ImFontAtlas::AddFontFromMemoryTTF(void* ttf_data, int ttf_size, float size_pixels, const ImFont_config* font_cfg_template, const ImWchar* glyph_ranges)
{
	ImFont_config font_cfg = font_cfg_template ? *font_cfg_template : ImFont_config();
...
ImFont* ImFontAtlas::AddFontFromMemoryCompressedTTF(const void* compressed_ttf_data, int compressed_ttf_size, float size_pixels, const ImFont_config* font_cfg_template, const ImWchar* glyph_ranges)
{
...
ImFont* ImFontAtlas::AddFontFromMemoryCompressedBase85TTF(const char* compressed_ttf_data_base85, float size_pixels, const ImFont_config* font_cfg, const ImWchar* glyph_ranges)
{
...
{
	IM_ASSERT(atlas->_configData.Size > 0);
	...
	for (int input_i = 0; input_i < atlas->_configData.Size; input_i++)
	{
		ImFont_config& cfg = atlas->_configData[input_i];
		if (!cfg.GlyphRanges)
	...
	ImFontTempBuildData* tmp_array = (ImFontTempBuildData*)ImGui::MemAlloc((size_t)atlas->_configData.Size * sizeof(ImFontTempBuildData));
	for (int input_i = 0; input_i < atlas->_configData.Size; input_i++)
	{
		ImFont_config& cfg = atlas->_configData[input_i];
...
void ImFontAtlasBuildSetupFont(ImFontAtlas* atlas, ImFont* font, ImFont_config* font_config, float ascent, float descent)
{
	if (!font_config->MergeMode)
	{
		font->ContainerAtlas = atlas;
		font->_configData = font_config;
		font->_configDataCount = 0;
