...
IMGUI_API bool ImGui::MyColorEdit3(const char* label, float col[4], ImGuiColorEditFlags flags) {
	float col4[4] = { col[0], col[1], col[2],  col[3] };
	if (!MyColorEdit4(label, col4, flags))
		return false;
	col[0] = col4[0]; col[1] = col4[1]; col[2] = col4[2], col[3] = col4[3];
	return true;
}
...
IMGUI_API bool ImGui::MyColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags) {
	...
	const float square_sz_with_spacing = (flags & ImGuiColorEditFlags_NoColorSquare2) ? 0.0f : (style.FramePadding.y + style.ItemInnerSpacing.x);

	// If no mode is specified, defaults to RGB
	if (!(flags & ImGuiColorEditFlags_ModeMask_2))
	...
	if (flags & ImGuiColorEditFlags_NoSliders2)
		flags = (flags & (~ImGuiColorEditFlags_ModeMask_2)) | ImGuiColorEditFlags_RGB | ImGuiColorEditFlags_NoOptions;

	// Read back edit mode from persistent storage
	if (!(flags & ImGuiColorEditFlags_NoOptions))
		flags = (flags & (~ImGuiColorEditFlags_ModeMask_2)) | (g.ColorEditModeStorage.GetInt(id, (flags & ImGuiColorEditFlags_ModeMask_2)) & ImGuiColorEditFlags_ModeMask_2);
	...
	bool alpha = (flags & ImGuiColorEditFlags_Alpha2) != 0;
	...
	if ((flags & (ImGuiColorEditFlags_RGB | ImGuiColorEditFlags_HSV)) != 0 && (flags & ImGuiColorEditFlags_NoSliders2) == 0) {
	...
	else if ((flags & ImGuiColorEditFlags_HEX) != 0 && (flags & ImGuiColorEditFlags_NoSliders2) == 0) {
	...
	if (!(flags & ImGuiColorEditFlags_NoColorSquare2)) {
		if (!(flags & ImGuiColorEditFlags_NoSliders2))
			ImGui::SameLine(0, style.ItemInnerSpacing.x);

		const ImVec4 col_display(col[0], col[1], col[2], 1.0f);
		if (ImGui::ColorButton2(col_display,false, false)) {
		...
		if (ImGui::BeginPopup("picker")) {
			...
			value_changed |= ImGui::MyColorPicker4("##picker", col, (flags & ImGuiColorEditFlags_Alpha2) | (ImGuiColorEditFlags_RGB | ImGuiColorEditFlags_HSV | ImGuiColorEditFlags_HEX));
			ImGui::PopItemWidth();
...
IMGUI_API bool ImGui::MyColorPicker3(const char* label, float col[3], ImGuiColorEditFlags flags) {
	...
	if (!MyColorPicker4(label, col4, flags & ~ImGuiColorEditFlags_Alpha2))
		return false;
...
bool ImGui::MyColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags) {
	...
	float bars_width = ImGui::GetWindowFontSize() * 1.5f;                                                           // Arbitrary smallish width of Hue/Alpha picking bars
	...
	if (!(flags & ImGuiColorEditFlags_NoSliders2)) {
		if ((flags & ImGuiColorEditFlags_ModeMask_2) == 0)
		...
		ImGuiColorEditFlags sub_flags = (alpha ? ImGuiColorEditFlags_Alpha2 : 0) | ImGuiColorEditFlags_NoPicker | ImGuiColorEditFlags_NoOptions | ImGuiColorEditFlags_NoColorSquare2;
