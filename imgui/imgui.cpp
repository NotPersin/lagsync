...
/*
 ...
 - Overwrite all the sources files except for im_config->h (if you have made modification to your copy of im_config->h)
 ...
 - You can later customize the im_config->h file to tweak some compilation time behavior, such as integrating imgui types with your own maths types.
 ...
 - 2017/05/26 (1.50) - removed ImFont_config::MergeGlyphCenterV in favor of a more multipurpose ImFont_config::GlyphOffset.
 ...
 - 2015/07/14 (1.43) - add new ImFontAtlas::AddFont() API. For the old AddFont***, moved the 'font_no' parameter of ImFontAtlas::AddFont** functions to the ImFont_config structure.
 ...
 A: Use the font atlas to pack them into a single texture:
	(Read extra_fonts/README.txt and the code in ImFontAtlas for more details.)
	  ...
	  // Options
	  ImFont_config _config;
	  _config->OversampleH = 3;
	  _config->OversampleV = 1;
	  _config->GlyphOffset.y -= 2.0f;      // Move everything by 2 pixels up
	  _config->GlyphExtraSpacing.x = 1.0f; // Increase spacing between characters
	  io.Fonts->LoadFromFileTTF("myfontfile.ttf", size_pixels, &_config);
	  ...
	  io.Fonts->LoadFromFileTTF("fontawesome-webfont.ttf", 16.0f, &_config, ranges); // Merge icon font
	  io.Fonts->LoadFromFileTTF("myfontfile.ttf", size_pixels, NULL, &_config, io.Fonts->GetGlyphRangesJapanese()); // Merge japanese glyphs
...
#ifdef __clang__
#pragma clang diagnostic ignored "-Wunknown-pragmas"        // warning : unknown warning group '-Wformat-pedantic *'        // not all warnings are known by all clang versions.. so ignoring warnings triggers new warnings on some _configuration. great!
...
// - or: Changing this variable to be TLS. You may #define GImGui in im_config->h for further custom hackery. Future development aim to make this context pointer explicit to all calls. Also read https://github.com/ocornut/imgui/issues/586
#ifndef GImGui
...
ImGuiStyle::ImGuiStyle() {
	...
	Colors[ImGuiCol_FrameBgHovered] = ImVec4(config->Misc.menucolor[0], config->Misc.menucolor[1], config->Misc.menucolor[2], 1.f);
	Colors[ImGuiCol_FrameBgActive] = ImVec4(config->Misc.menucolor[0], config->Misc.menucolor[1], config->Misc.menucolor[2], 1.f);
	Colors[ImGuiCol_TitleBg] = ImVec4(0.27f, 0.27f, 0.54f, 0.83f);
	Colors[ImGuiCol_TitleBgCollapsed] = ImVec4(0.40f, 0.40f, 0.80f, 0.20f);
	Colors[ImGuiCol_TitleBgActive] = ImVec4(0.32f, 0.32f, 0.63f, 0.87f);
	Colors[ImGuiCol_MenuBarBg] = ImVec4(0.40f, 0.40f, 0.55f, 0.80f);
	Colors[ImGuiCol_ScrollbarBg] = ImVec4(0.20f, 0.25f, 0.30f, 0.60f);
	Colors[ImGuiCol_ScrollbarGrab] = ImVec4(config->Misc.menucolor[0], config->Misc.menucolor[1], config->Misc.menucolor[2], 1.f);
	Colors[ImGuiCol_ScrollbarGrabHovered] = ImVec4(config->Misc.menucolor[0], config->Misc.menucolor[1], config->Misc.menucolor[2], 1.f);
	Colors[ImGuiCol_ScrollbarGrabActive] = ImVec4(config->Misc.menucolor[0], config->Misc.menucolor[1], config->Misc.menucolor[2], 1.f);
	Colors[ImGuiCol_ComboBg] = ImVec4(0.20f, 0.20f, 0.20f, 0.99f);
	Colors[ImGuiCol_CheckMark] = ImVec4(0.90f, 0.90f, 0.90f, 0.50f);
	Colors[ImGuiCol_SliderGrab] = ImVec4(1.00f, 1.00f, 1.00f, 0.30f);
	Colors[ImGuiCol_SliderGrabActive] = ImVec4(config->Misc.menucolor[0], config->Misc.menucolor[1], config->Misc.menucolor[2], 1.f);
	Colors[ImGuiCol_Button] = ImVec4(0.67f, 0.40f, 0.40f, 0.60f);
	Colors[ImGuiCol_ButtonHovered] = ImVec4(config->Misc.menucolor[0], config->Misc.menucolor[1], config->Misc.menucolor[2], 1.f);
	Colors[ImGuiCol_ButtonActive] = ImVec4(config->Misc.menucolor[0], config->Misc.menucolor[1], config->Misc.menucolor[2], 1.f);
	Colors[ImGuiCol_Header] = ImVec4(config->Misc.menucolor[0], config->Misc.menucolor[1], config->Misc.menucolor[2], 1.f);
	Colors[ImGuiCol_HeaderHovered] = ImVec4(config->Misc.menucolor[0], config->Misc.menucolor[1], config->Misc.menucolor[2], 1.f);
	Colors[ImGuiCol_HeaderActive] = ImVec4(config->Misc.menucolor[0], config->Misc.menucolor[1], config->Misc.menucolor[2], 1.f);
	...
	Colors[ImGuiCol_TextSelectedBg] = ImVec4(config->Misc.menucolor[0], config->Misc.menucolor[1], config->Misc.menucolor[2], 1.f);
...
static void AddDrawListToRenderList(ImVector<ImDrawList*>& out_render_list, ImDrawList* draw_list) {
	...
	// A) Add '#define ImDrawIdx unsigned int' in im_config->h to set the index size to 4 bytes. You'll need to handle the 4-bytes indices to your renderer.
...
//ImColor(_config->Misc.menucolor[0], _config->Misc.menucolor[1], _config->Misc.menucolor[2], 1.0));
void ImGui::VRenderText(ImVec2 pos, const char* text, const char* text_end, bool hide_text_after_hash, ImColor custom) {
	ImGuiContext& g = *GImGui;
	ImGuiWindow* window = GetCurrentWindow();
	...
	const int text_len = (int)(text_display_end - text);
	if (text_len > 0) {
		//window->DrawList->AddText(g.Font, g.FontSize, ImVec2(pos.x + 1, pos.y + 1), custom, text, text_display_end);
		window->DrawList->AddText(g.Font, g.FontSize, pos, custom, text, text_display_end);
		if (g.LogEnabled)
			LogRenderedText(pos, text, text_display_end);
	}
}
...
void ImGui::RenderTextWrapped(ImVec2 pos, const char* text, const char* text_end, float wrap_width) {
	...
	if (text_len > 0) {
		...
		bool thovered, theld;
		std::string backup = text; backup += "##test";
		const ImGuiID id3 = window->GetID(backup.c_str());
		bool tpressed = ButtonBehavior(ImRect(ImVec2(pos.x + 10, pos.y + 5), ImVec2(pos.x + g.FontSize + 100, pos.y + 15)), id3, &thovered, &theld);
		if (thovered)
			window->DrawList->AddText(g.Font, g.FontSize, pos, GetColorU32(ImVec4(config->Misc.accentcolor[0], config->Misc.accentcolor[1], config->Misc.accentcolor[2], 1.f)), text, text_end, wrap_width);
		else
...
void ImGui::VRenderTextClipped(const ImVec2& pos_min, const ImVec2& pos_max, const char* text, const char* text_end, const ImVec2* text_size_if_known, const ImVec2& align, const ImRect* clip_rect, ImColor color) {
	// Hide anything after a '##' string
	const char* text_display_end = FindRenderedTextEnd(text, text_end);
	const int text_len = (int)(text_display_end - text);
	if (text_len == 0)
	...
	if (need_clipping) {
		ImVec4 fine_clip_rect(clip_min->x, clip_min->y, clip_max->x, clip_max->y);
		//window->DrawList->AddText(g.Font, g.FontSize, ImVec2(pos.x + 1, pos.y + 1), GetColorU32(ImVec4(0, 0, 0, 1)), text, text_display_end, 0.0f, &fine_clip_rect);
		window->DrawList->AddText(g.Font, g.FontSize, pos, color, text, text_display_end, 0.0f, &fine_clip_rect);
	}
	else {
		//window->DrawList->AddText(g.Font, g.FontSize, ImVec2(pos.x + 1, pos.y + 1), GetColorU32(ImVec4(0, 0, 0, 1)), text, text_display_end, 0.0f, NULL);
		window->DrawList->AddText(g.Font, g.FontSize, pos, color, text, text_display_end, 0.0f, NULL);
	}
	if (g.LogEnabled)
		LogRenderedText(pos, text, text_display_end);
}
...
void ImGui::RenderFrameBorder(ImVec2 p_min, ImVec2 p_max, float rounding) {
	...
	if (window->Flags & ImGuiWindowFlags_ShowBorders) {
		window->DrawList->AddRect(p_min + ImVec2(1, 1), p_max + ImVec2(1, 1), GetColorU32(ImGuiCol_BorderShadow), rounding);
		window->DrawList->AddRect(p_min, p_max, GetColorU32(ImVec4(config->Misc.menucolor[0], config->Misc.menucolor[1], config->Misc.menucolor[2], 1.f)), rounding);
	}
}
...
void ImGui::EndChild(bool shadow) {
	...
	if (window->ScrollbarY) {
		...
		auto min_bot = min_top + ImVec2(0, GetWindowSize().y - 23);
...
bool ImGui::Begin(const char* name, bool* p_open, const ImVec2& size_on_first_use, float bg_alpha, ImGuiWindowFlags flags) {
	...
	if (first_begin_of_the_frame) {
		...
		if (window->Collapsed) {
			...
			//	window->DrawList->AddRectFilled(window->Pos + ImVec2(-1, window->TitleBarHeight() - 1), window->Pos + window->Size + ImVec2(1, 1), ColorConvertFloat4ToU32(style.Colors[ImGuiCol_BorderShadow]), window_rounding, (flags & ImGuiWindowFlags_NoTitleBar) ? ImGuiCorner_All : ImGuiCorner_BottomLeft | ImGuiCorner_BottomRight);

			if (bg_color.w > 0.0f && (flags & ImGuiWindowFlags_Popup) == 0)
			...
			// Borders
			if (flags & ImGuiWindowFlags_ShowBorders && (flags & ImGuiWindowFlags_Popup) == 0) {
				if ((flags & ImGuiWindowFlags_ChildWindow) != 0) {
					window->DrawList->AddRect(window->Pos, window->Pos + window->Size - ImVec2(8, 8), GetColorU32(ImVec4(0.23, 0.23, 0.23, 1.f)), window_rounding);
				}
				else {
					window->DrawList->AddRect(window->Pos - ImVec2(2, 2), window->Pos + window->Size + ImVec2(2, 2), GetColorU32(ImVec4(0.23, 0.23, 0.23, 0.95f)), window_rounding);
					window->DrawList->AddRect(window->Pos - ImVec2(1, 1), window->Pos + window->Size + ImVec2(1, 1), GetColorU32(ImVec4(0.17, 0.17, 0.17, 1.f)), window_rounding);
					window->DrawList->AddRect(window->Pos, window->Pos + window->Size, GetColorU32(ImVec4(0.23, 0.23, 0.23, 1.f)), window_rounding);
				}
				if (flags & ImGuiWindowFlags_Popup) {
				//	window->DrawList->AddRect(window->Pos, window->Pos + window->Size, GetColorU32(ImVec4(0.07, 0.07, 0.07, 0.0)), window_rounding);
				}
				if (!(flags & ImGuiWindowFlags_NoTitleBar))
					window->DrawList->AddLine(title_bar_rect.GetBL() + ImVec2(1, 0), title_bar_rect.GetBR() - ImVec2(1, 0), GetColorU32(ImVec4(0.224, 0.208, 0.243, 1.f)));
			}
		}
	...
	if (window->ScrollbarY && (flags & ImGuiWindowFlags_Popup) == 0)
	...
	clip_rect.Max.y = ImFloor(0.5f + window->Pos.y + window->Size.y - window->ScrollbarSizes.y - border_size - 8);
	PushClipRect(clip_rect.Min, clip_rect.Max, false);
	...
	if (flags & ImGuiWindowFlags_ChildWindow) {
		...
		if (!(flags & ImGuiWindowFlags_AlwaysAutoResize) && window->AutoFitFramesX <= 0 && window->AutoFitFramesY <= 0)
			window->Collapsed |= (window->WindowRectClipped.Min.x >= window->WindowRectClipped.Max.x || window->WindowRectClipped.Min.y >= window->WindowRectClipped.Max.y - 8);
...
static void Scrollbar(ImGuiWindow* window, bool horizontal) {
	...
	// Render
	const ImU32 grab_col = ImGui::GetColorU32(held || hovered ? ImVec4(config->Misc.accentcolor[0], config->Misc.accentcolor[1], config->Misc.accentcolor[2],1.f) : ImVec4(0.5f, 0.5f, 0.5f, 0.5f));
	if (horizontal)
...
void ImGui::TextUnformatted(const char* text, const char* text_end) {
	...
	if (text_end - text > 2000 && !wrap_enabled) {
		...
		const ImVec2 text_pos = window->DC.CursorPos + ImVec2(30.0f, window->DC.CurrentLineTextBaseOffset);
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags) {
	...
	const ImRect bb(pos, pos + size - ImVec2(10,0));
	ItemSize(bb, style.FramePadding.y);
	if (!ItemAdd(bb, &id))
		return false;
	...
	auto menu_light = ImGui::GetColorU32(ImVec4(config->Misc.menucolor[0], config->Misc.menucolor[1], config->Misc.menucolor[2], 0.95));
	auto menu_dark = ImGui::GetColorU32(ImVec4(config->Misc.menucolor[0] - 0.3, config->Misc.menucolor[1] - 0.3, config->Misc.menucolor[2] - 0.3, 0.95));
	...
	if ((held || hovered))
	{
		VRenderTextClipped(bb.Min - ImVec2(0, 4), bb.Max - style.FramePadding, label, NULL, &label_size, style.ButtonTextAlign, &bb, ImVec4(config->Misc.accentcolor[0], config->Misc.accentcolor[1], config->Misc.accentcolor[2], 1.f));
	}
	else {
		VRenderTextClipped(bb.Min - ImVec2(0, 4), bb.Max - style.FramePadding, label, NULL, &label_size, style.ButtonTextAlign, &bb, ImVec4(1.f, 1.f, 1.f, 1.f));
	}
	return pressed;
}
...
void ImGui::Tab_ButtonEx(const char* label, int& enabled, int best, const ImVec2& size_arg , ImGuiButtonFlags flags) {
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return;
	...
	// Render
	const auto coloorr = ImGui::GetColorU32(ImVec4(config->Misc.menucolor[0], config->Misc.menucolor[1], config->Misc.menucolor[2], 255));
	//RenderFrameBorder(bb.Min + ImVec2(0, 5), bb.Max - ImVec2(0, 5));
	...
	ImColor colr = (enabled == best) ? ImGui::GetColorU32(ImVec4(config->Misc.menucolor[0], config->Misc.menucolor[1], config->Misc.menucolor[2], 255)) : ImGui::GetColorU32(ImGuiCol_Text);

	VRenderTextClipped(bb.Min + style.FramePadding - ImVec2(0, 3), bb.Max - style.FramePadding, label, NULL, &label_size, style.ButtonTextAlign, &bb, colr);

	if (pressed)
		enabled = best;
}

void ImGui::Tab_Button(const char* label, int& enabled, int best, const ImVec2& size) {
	return Tab_ButtonEx( label,  enabled,  best ,size);
}

bool ImGui::Tab(const char* label, bool toggled, const ImVec2& size_arg)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	const ImRect bb(pos, pos + size + ImVec2(0,10));
	ItemSize(bb, style.FramePadding.y);
	if (!ItemAdd(bb, &id))
		return false;
	bool hovered, held;
	...
	// Render
	const ImU32 col = GetColorU32((hovered || held || toggled) ? ImGuiCol_ButtonActive : ImGuiCol_Button);
	//RenderFrame(bb.Min, bb.Max, col, toggled);//style.FrameRounding);
	VRenderTextClipped(bb.Min, bb.Max, label, NULL, &label_size, style.ButtonTextAlign, NULL, col);
...
void ImGui::Switch(const char* label, const char* const* items, const ImVec2& size_arg, int array_size, int& cfg)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return;
	...
	std::string buf = label; buf += "##";
	ImGuiContext& g = *GImGui;
	const ImGuiStyle& style = g.Style;
	const ImGuiID id = window->GetID(label);
	const ImGuiID id2 = window->GetID(buf.c_str());
	const ImVec2 label_size = CalcTextSize(items[returned], NULL, true);
	const ImVec2 plus_size = CalcTextSize("+", NULL, true);
	const ImVec2 minus_size = CalcTextSize("-", NULL, true);

	ImVec2 pos = window->DC.CursorPos + ImVec2(0,0);
	ImVec2 size = CalcItemSize(size_arg, label_size.x + plus_size.x + minus_size.x + style.FramePadding.x * 2.0f, label_size.y + style.FramePadding.y * 2.0f);
	...
	const ImRect minus_bb(bb.Min, bb.Max - ImVec2(40, 0));
	bool pressed_minus = ButtonBehavior(minus_bb, id, &hovered, &held, 0);

	const ImRect plus_bb(bb.Min + ImVec2(25, 0), bb.Max + ImVec2(10, 0));
	bool pressed_plus = ButtonBehavior(plus_bb, id2, &hovered2, &held2, 0);

	if (pressed_minus)
		returned--;
	else if (pressed_plus)
		returned++;

	if (returned > array_size - 1)
		returned = array_size - 1;
	else if (returned < 0)
		returned = 0;

	// Render
	const ImU32 col = GetColorU32( ImGuiCol_ButtonActive);
	//RenderFrame(bb.Min, bb.Max, col, toggled);//style.FrameRounding);

	VRenderTextClipped(bb.Min - ImVec2(label_size.x / 2 + 23,0), bb.Max, "-", NULL, &plus_size, style.ButtonTextAlign, NULL, col);
	VRenderTextClipped(bb.Min + ImVec2((12 - label_size.x / 2) ,0), bb.Max, items[returned], NULL, &label_size, style.ButtonTextAlign, NULL, col);
	VRenderTextClipped(bb.Min + ImVec2(25 + (label_size.x / 2), 0), bb.Max, "+", NULL, &plus_size, style.ButtonTextAlign, NULL, col);


	//VRenderTextClipped(bb.Min + ImVec2(minus_size.x + label_size.x + 3, -10), bb.Max, std::to_string(returned).c_str(), NULL, &label_size, style.ButtonTextAlign, NULL, col);
...
bool ImGui::SliderBehavior(const ImRect& frame_bb, ImGuiID id, const char* display_format, float* v, float v_min, float v_max, float power, int decimal_precision, ImGuiSliderFlags flags) {
	...
	window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, GetColorU32(ImVec4(0.06, 0.06, 0.06, 1.0)), 0.f);
	//window->DrawList->AddRectFilled(frame_bb.Min + ImVec2(1, -2) + ImVec2(0, 1), frame_bb.Max - ImVec2(1, 6), GetColorU32(ImVec4(0.07, 0.07, 0.07, 1.0)), 0.f);
	...
	window->DrawList->AddRectFilled(frame_bb.Min, grab_bb.Max, GetColorU32(ImVec4(config->Misc.menucolor[0], config->Misc.menucolor[1], config->Misc.menucolor[2], 255))); // Main gradient.
	//->DrawList->AddRectFilledMultiColor(frame_bb.Min + ImVec2(1, -2) + ImVec2(0, 2), grab_bb.Max - ImVec2(-1, 6) + ImVec2(0, 2), GetColorU32(ImVec4(0.0f, 0.0f, 0.0f, 0.05f)), GetColorU32(ImVec4(0.0f, 0.0f, 0.0f, 0.05f)), GetColorU32(ImVec4(0.0f, 0.0f, 0.0f, 0.38f)), GetColorU32(ImVec4(0.0f, 0.0f, 0.0f, 0.38f))); // Main gradient.

		char value_buf[64];
	const char* value_buf_end = value_buf + ImFormatString(value_buf, IM_ARRAYSIZE(value_buf), display_format, *v);
	ImVec2 item_size = CalcTextSize(value_buf, NULL, true);
	RenderText(grab_bb.Max - ImVec2(item_size.x / 2 + *v > 99 ? 3 : 0 , 0) - ImVec2(2, 10), value_buf, value_buf_end);

	return value_changed;
}

bool ImGui::SliderBehavior(const ImRect& frame_bb, ImGuiID id, const char* display_format, const char* label, float* v, float v_min, float v_max, float power, int decimal_precision, ImGuiSliderFlags flags) {
	ImGuiContext& g = *GImGui;
	ImGuiWindow* window = GetCurrentWindow();
	const ImGuiStyle& style = g.Style;
	...
	window->DrawList->AddRectFilledMultiColor(frame_bb.Min - ImVec2(-1, 2), frame_bb.Max - ImVec2(2, 5), color_grey, color_grey, color_black, color_black);
	//window->DrawList->AddRectFilled(frame_bb.Min + ImVec2(1, -2) + ImVec2(0, 1), frame_bb.Max - ImVec2(1, 6), GetColorU32(ImVec4(0.07, 0.07, 0.07, 1.0)), 0.f);
	...
	window->DrawList->AddRectFilledMultiColor(frame_bb.Min + ImVec2(1, -2), grab_bb.Max - ImVec2(0, 5), menu_light, menu_light, menu_dark, menu_dark);
	//->DrawList->AddRectFilledMultiColor(frame_bb.Min + ImVec2(1, -2) + ImVec2(0, 2), grab_bb.Max - ImVec2(-1, 6) + ImVec2(0, 2), GetColorU32(ImVec4(0.0f, 0.0f, 0.0f, 0.05f)), GetColorU32(ImVec4(0.0f, 0.0f, 0.0f, 0.05f)), GetColorU32(ImVec4(0.0f, 0.0f, 0.0f, 0.38f)), GetColorU32(ImVec4(0.0f, 0.0f, 0.0f, 0.38f))); // Main gradient.

	char value_buf[64];
	const char* value_buf_end = value_buf + ImFormatString(value_buf, IM_ARRAYSIZE(value_buf), display_format, *v);
	ImVec2 item_size = CalcTextSize(value_buf, NULL, true);
	RenderText(frame_bb.Min - ImVec2(item_size.x / 2 + 13, 5), value_buf, value_buf_end);
...
bool ImGui::SliderFloat(const char* label, float* v, float v_min, float v_max, const char* display_format, float power) {
	...
	const ImVec2 label_size = CalcTextSize(label, NULL, true);
	const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w + 0, label_size.y));
	const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x + 45, 0) + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 2.0f));
	...
	const ImRect total_bbb(total_bb.Min + ImVec2(label_size.x + 45, 0), total_bb.Max + ImVec2(label_size.x + 45, 0));
	const bool hovered = IsHovered(total_bbb, id);
	if (hovered)
		SetHoveredID(id);
	...
	const bool value_changed = SliderBehavior( ImRect(ImVec2(frame_bb.Min.x + label_size.x + 45, frame_bb.Min.y + 1), ImVec2(frame_bb.Max.x + 93, frame_bb.Max.y - 1)), id, display_format, label, v, v_min, v_max, power, decimal_precision);
	...
	if (label_size.x > 0.0f) {
		bool thovered, theld;
		std::string backup = label; backup += "##test";
		const ImGuiID id3 = window->GetID(backup.c_str());
		bool tpressed = ButtonBehavior(frame_bb, id3, &thovered, &theld);

		if (thovered)
			VRenderText(ImVec2(frame_bb.Min.x + 15, frame_bb.Min.y - 5), label, nullptr, true, GetColorU32(ImVec4(config->Misc.accentcolor[0], config->Misc.accentcolor[1], config->Misc.accentcolor[2], 1.f)));
		else		
			VRenderText(ImVec2(frame_bb.Min.x + 15, frame_bb.Min.y - 5), label, nullptr, true, ImColor(255, 255, 255));
	}
	return value_changed;
}
...
bool ImGui::VSliderFloat(const char* label, const ImVec2& size, float* v, float v_min, float v_max, const char* display_format, float power) {
	...
	RenderTextClipped(ImVec2(frame_bb.Min.x, frame_bb.Min.y), frame_bb.Max, value_buf, value_buf_end, NULL, ImVec2(0.5f, 0.0f));
	if (label_size.x > 0.0f)
...
bool ImGui::Checkbox(const char* label, bool* v, int colorpicker) {
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	if (colorpicker > 0)
		check_bb = ImRect(window->DC.CursorPos + ImVec2(285 - (colorpicker * 20), 0), window->DC.CursorPos + ImVec2( 285 - (colorpicker * 20) + label_size.y + style.FramePadding.y * 0.5, label_size.y + style.FramePadding.y * 0.5));
	else
		check_bb = ImRect(window->DC.CursorPos + ImVec2(285, 0), window->DC.CursorPos + ImVec2( 285 + label_size.y + style.FramePadding.y * 0.5, label_size.y + style.FramePadding.y * 0.5));
	ItemSize(check_bb, style.FramePadding.y * 0.5);
	
	ImRect total_bb = check_bb;
	if (label_size.x > 0)
		SameLine(0, style.ItemInnerSpacing.x);
	const ImRect text_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(0, style.FramePadding.y * 0.5) + label_size);
	if (label_size.x > 0) {
		ItemSize(ImVec2(text_bb.GetWidth(), check_bb.GetHeight() * 1.5), style.FramePadding.y);
	...
	//RenderTextClipped(check_bb.Min + style.FramePadding, check_bb.Max - style.FramePadding, label, NULL, &label_size, style.ButtonTextAlign, &check_bb);
	window->DrawList->AddRect(check_bb.Min, check_bb.Max, GetColorU32(ImVec4(0.6f, 0.6f, 0.6f, 1.0f)));

	window->DrawList->AddRectFilledMultiColor(check_bb.Min, check_bb.Max, color_grey, color_grey, color_black, color_black); // Main gradient.

	if (*v) {
		window->DrawList->AddRectFilledMultiColor(check_bb.Min, check_bb.Max, menu_light, menu_light, menu_dark, menu_dark);
	}
	...
	if (label_size.x > 0.0f) {
		...
		std::string backup = label; backup += "##test";
		const ImGuiID id3 = window->GetID(backup.c_str());
		bool tpressed = ButtonBehavior(text_bb, id3, &thovered, &theld);

		if (thovered)
			VRenderText(text_bb.GetTL() - ImVec2(0, 3), label, nullptr, true, GetColorU32(ImVec4(config->Misc.accentcolor[0], config->Misc.accentcolor[1], config->Misc.accentcolor[2], 1.f)));
		else
			VRenderText(text_bb.GetTL() - ImVec2(0, 3), label, nullptr, true, ImColor(255, 255, 255));
...
bool ImGui::RadioButton(const char* label, bool active) {
	...
	if (window->Flags & ImGuiWindowFlags_ShowBorders) {
		window->DrawList->AddCircle(center + ImVec2(1, 1), radius, GetColorU32(ImVec4(config->Misc.menucolor[0], config->Misc.menucolor[1], config->Misc.menucolor[2], 1.f)), 16);
		window->DrawList->AddCircle(center, radius, GetColorU32(ImVec4(config->Misc.menucolor[0], config->Misc.menucolor[1], config->Misc.menucolor[2], 1.f)), 16);
	}
...
bool ImGui::InputTextEx(const char* label, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiTextEditCallback callback, void* user_data) {
	...
	const ImGuiID id = window->GetID(label);
	const ImVec2 label_size = CalcTextSize(label, NULL, true);
	ImVec2 size = CalcItemSize(size_arg, CalcItemWidth(), (is_multiline ? GetTextLineHeight() * 8.0f : 0) + style.FramePadding.y * 2.0f + 14); // Arbitrary default of 8 lines high for multi-line
	const ImRect frame_bb(window->DC.CursorPos + ImVec2(14, 0), window->DC.CursorPos + size - ImVec2(55, -2));
	...
	// Render
	auto color_grey = ImGui::GetColorU32(ImVec4(0.25, 0.25, 0.25, 0.65));
	auto color_black = ImGui::GetColorU32(ImVec4(0.1, 0.1, 0.1, 0.45));
	...
	if (!is_multiline) {
		...
		window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, GetColorU32(ImVec4(0.4f, 0.4f, 0.4f, 1.0f)));
		window->DrawList->AddRectFilledMultiColor(frame_bb.Min, frame_bb.Max, color_grey, color_grey, color_black, color_black); // Main gradient.	
	
	}
	const ImVec4 clip_rect(frame_bb.Min.x, frame_bb.Min.y, frame_bb.Min.x + size.x, frame_bb.Min.y + size.y); // Not using frame_bb.Max because we have adjusted size
	ImVec2 render_pos = is_multiline ? draw_window->DC.CursorPos : ImVec2(frame_bb.Min.x + 70, frame_bb.Min.y) + style.FramePadding;
	...
	if (g.ActiveId == id || is_currently_scrolling) {
		...
		auto true_size = CalcTextSize(buf_display);
		draw_window->DrawList->AddText(g.Font, g.FontSize, render_pos - render_scroll - ImVec2(true_size.x / 2, 0), GetColorU32(ImGuiCol_Text), buf_display, buf_display + edit_state.CurLenA, 0.0f, is_multiline ? NULL : &clip_rect);

		// Draw blinking cursor
		bool cursor_is_visible = (g.InputTextState.CursorAnim <= 0.0f) || fmodf(g.InputTextState.CursorAnim, 1.20f) <= 0.80f;
		const char* buf_end = NULL;
		auto true_size_rect = CalcTextSize(buf_display, buf_end);
		ImVec2 cursor_screen_pos = render_pos + cursor_offset - render_scroll - ImVec2(true_size_rect.x / 2, 0);
		...
		auto true_size = CalcTextSize(buf_display, buf_end);
		 draw_window->DrawList->AddText(g.Font, g.FontSize, render_pos - ImVec2(true_size.x / 2, 0), GetColorU32(ImGuiCol_Text), buf_display, buf_end, 0.0f, is_multiline ? NULL : &clip_rect);
	}
	...
	//if (label_size.x > 0)
	//	RenderText(ImVec2(frame_bb.Max.x - (label_size.x / 2), frame_bb.Min.y + style.FramePadding.y), label);
...
bool ImGui::InputScalarEx(const char* label, ImGuiDataType data_type, void* data_ptr, void* step_ptr, void* step_fast_ptr, const char* scalar_format, ImGuiInputTextFlags extra_flags) {
	...
	BeginGroup();
	PushID(label);
	const ImVec2 button_sz = ImVec2(0, 0);
	//if (step_ptr)
	//	PushItemWidth(ImMax(.35f, ImVec2(85, 0)));
	...
	if (label_size.x > 0) {
		SameLine(0, style.ItemInnerSpacing.x);
		RenderText(ImVec2(window->DC.CursorPos.x - label_size.x + 153, window->DC.CursorPos.y + style.FramePadding.y), label);
		ItemSize(label_size + ImVec2(153, 0), style.FramePadding.y);
	}
	EndGroup();
...
bool ImGui::Hotkey(const char* label, int* k, const ImVec2& size_arg) {
	...
	const ImGuiID id = window->GetID(label);
	const ImVec2 label_size = CalcTextSize(label, NULL, true);
	ImVec2 size = ImVec2(83, 18);
	const ImRect frame_bb(window->DC.CursorPos - ImVec2(label_size.x + 45, 0.0f), window->DC.CursorPos + size);
	...
	if (g.ActiveId == id) {
		...
		if (!value_changed) {
			...
			else if (GetAsyncKeyState(0xA5)) {
				key = 0xA5;
				value_changed = true;
				ClearActiveID();
			}
			else if (GetAsyncKeyState(0xA4)) {
				key = 0xA4;
				value_changed = true;
				ClearActiveID();
			}
			else {
				for (auto i = VK_LBUTTON; i < VK_RMENU; i++) {
					if (io.KeysDown[i]) {
						key = i;
						value_changed = true;
						ClearActiveID();
	...
	const ImVec2 KeyNamesSize = CalcTextSize(buf_display, NULL, true);
	RenderTextClipped(frame_bb.Min - ImVec2(KeyNamesSize.x - 45 - style.KeyBindOverride.x, -1 - style.KeyBindOverride.y), frame_bb.Max, buf_display, NULL, NULL);
...
bool ImGui::Combo(const char* label, int* current_item, const char* const* items, int items_count, int height_in_items, int color ) {
	const bool value_changed = Combo(label, current_item, Items_ArrayGetter, (void*)items, items_count, height_in_items, color);
	ImGuiStyle& style = ImGui::GetStyle();
...
bool ImGui::Combo(const char* label, int* current_item, const char* items_separated_by_zeros, int height_in_items, int color) {
	...
	bool value_changed = Combo(label, current_item, Items_SingleStringGetter, (void*)items_separated_by_zeros, items_count, (height_in_items == -1 ? items_count : height_in_items, color));
	return value_changed;
}
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImVec2 popup_size)
{
	...
	const ImVec2 label_size = CalcTextSize(label, NULL, true);
	const ImRect frame_bb(window->DC.CursorPos + ImVec2(18, 0), window->DC.CursorPos + ImVec2(w + 0, label_size.y + style.FramePadding.y * 2.0f - 1));
	const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f) + ImVec2(150, 0));
	ItemSize(total_bb, style.FramePadding.y);
	if (!ItemAdd(total_bb, &id))
		return false;
	...
	const ImRect textbb(ImVec2(frame_bb.Min.x - 6, frame_bb.Min.y - 15), ImVec2(frame_bb.Min.x + 280, frame_bb.Min.y + 5));

	bool pressed = ButtonBehavior(textbb, id, &hovered, &held);

	bool popup_open = IsPopupOpen(id);

	if (preview_value != NULL) {
		RenderText(ImVec2(frame_bb.Min.x + 268, frame_bb.Min.y - 10), preview_value);
	}
	if (label_size.x > 0)
		RenderText(ImVec2(frame_bb.Min.x - 3, frame_bb.Min.y - 10), label);
	...
	if (popup_open) {
		...
		if ((popup_y2 - popup_y1) < ImMin(popup_size.y, frame_bb.Min.y - style.DisplaySafeAreaPadding.y))
		{
			...
			SetNextWindowPos(ImVec2(frame_bb.Min.x + 110, frame_bb.Min.y - 10), ImGuiCond_Always);
		}
		else
		{
		...
		const ImGuiWindowFlags flags = ImGuiWindowFlags_ComboBox | ImGuiWindowFlags_ShowBorders;
...
//Combo box function.
bool ImGui::Combo(const char* label, int* current_item, bool(*items_getter)(void*, int, const char**), void* data, int items_count, int height_in_items, int color) {
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	const ImGuiID id = window->GetID(label);
	const float w = 298;//CalcItemWidth();

	const ImVec2 label_size = CalcTextSize(label, NULL, true);
	const ImRect frame_bb(window->DC.CursorPos + ImVec2(0, -6), window->DC.CursorPos + ImVec2(w + style.ComboOverride.x + 0, label_size.y + style.FramePadding.y * 0.5f));
	const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2( label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f + style.FramePadding.y * 0.5f  ));
	const ImU32 frame_col = GetColorU32(ImVec4(0.224, 0.208, 0.243, 1.0)); // MAKRER 1
	...
	//window->DrawList->AddRectFilled(frame_bb.Min + ImVec2(,0), ImVec2(value_x2, frame_bb.Max.y) + ImVec2(-9, 1), GetColorU32(ImVec4(0.07, 0.07, 0.07, 1.0)),  style.FrameRounding, 0);
	//window->DrawList->AddRectFilled(frame_bb.Min + ImVec2(1, 1), ImVec2(value_x2, frame_bb.Max.y) + ImVec2(-9, 0), GetColorU32(ImVec4(0.09, 0.09, 0.09, 1.0)), style.FrameRounding, 0);
	...
	if (label_size.x > 0) {
		...
		if (thovered)
			VRenderTextClipped(frame_bb.Min + ImVec2(15,0), value_bb.Max, label, NULL, NULL, ImVec2(0.0f, 0.0f), NULL, ImColor(config->Misc.accentcolor[0], config->Misc.accentcolor[1], config->Misc.accentcolor[2], 1.f));
		else				
			VRenderTextClipped(frame_bb.Min + ImVec2(15,0), value_bb.Max, label, NULL, NULL, ImVec2(0.0f, 0.0f), NULL, ImColor(255, 255, 255));

	}
	if (*current_item >= 0 && *current_item < items_count) {
		const char* item_text;
		if (items_getter(data, *current_item, &item_text)) {
			item_size = CalcTextSize(item_text, NULL, true);
			VRenderTextClipped(frame_bb.Min - ImVec2(item_size.x - style.ComboOverride.x, 0) + ImVec2(w, 0), value_bb.Max + ImVec2(((item_size.x)), 0), item_text, NULL, NULL, ImVec2(0.0f, 0.0f), NULL, ImColor(1.f, 1.f, 1.f, 1.0));
		}
	}
	...
	//	VRenderText(ImVec2(frame_bb.Min.x, frame_bb.Min.y), label, "", false, ImColor(1.f, 1.f, 1.f, 1.0));
	...
	if (IsPopupOpen(id)) {
		...
		ImRect popup_rect(ImVec2(frame_bb.Min.x, popup_y1 -16 ), ImVec2(frame_bb.Max.x, popup_y2 + 15.f));
		SetNextWindowPos(popup_rect.Min);
		SetNextWindowSize(popup_rect.GetSize() + ImVec2(0, 0));
		PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(0, 0));

		const ImGuiWindowFlags flags = ImGuiWindowFlags_ComboBox | ((window->Flags & ImGuiWindowFlags_ShowBorders) ? ImGuiWindowFlags_ShowBorders : ImGuiWindowFlags_ShowBorders);
		if (BeginPopupEx(id, flags)) {
			...
			for (int i = 0; i < items_count; i++) {
				...
				SetCursorPosX(GetCursorPosX() + 206 - item_size.x - style.ComboOverride.x);
				styling.ComboOverride.x = 0;
...
//
//// Combo box helper allowing to pass an array of strings.
//bool ImGui::MultiCombo(const char* label, bool** current_item, const char* const* items, int items_count, int height_in_items, int color) {
//	const bool value_changed = MultiCombo(label, current_item, Items_ArrayGetter, (void*)items, items_count, height_in_items, color);
//	ImGuiStyle& style = ImGui::GetStyle();
//	style.ComboOverride.x = 0;
//	return value_changed;
//}
//
////Combo box function.
//bool ImGui::MultiCombo(const char* label, bool** current_item, bool(*items_getter)(void*, bool, const char**), void* data, int items_count, int height_in_items, int color) {
//	ImGuiWindow* window = GetCurrentWindow();
//	if (window->SkipItems)
//		return false;
//
...
//
//		if (thovered)
//			VRenderTextClipped(frame_bb.Min + ImVec2(15, 0), value_bb.Max, label, NULL, NULL, ImVec2(0.0f, 0.0f), NULL, ImColor(_config->Misc.accentcolor[0], _config->Misc.accentcolor[1], _config->Misc.accentcolor[2], 1.f));
//		else
//			VRenderTextClipped(frame_bb.Min + ImVec2(15, 0), value_bb.Max, label, NULL, NULL, ImVec2(0.0f, 0.0f), NULL, ImColor(255, 255, 255));
//
//	}
//
//	std::string final_text = "none";
//
//	for (int x = 0; x < items_count; x++) {
//		const char* item_text;
//		if (items_getter(data, current_item[x], &item_text)) {
//			item_size = CalcTextSize(item_text, NULL, true);
//		}
//		if (final_text == "none") {
//			final_text = item_text; final_text += "";
//		}
//		else {
//			final_text += ", ";
...
//	}
//
//	item_size = CalcTextSize(final_text.c_str(), NULL, true);
//	VRenderTextClipped(frame_bb.Min - ImVec2(item_size.x - style.ComboOverride.x, 0) + ImVec2(w, 0), value_bb.Max + ImVec2(((item_size.x)), 0), final_text.c_str(), NULL, NULL, ImVec2(0.0f, 0.0f), NULL, ImColor(1.f, 1.f, 1.f, 1.0));
//
//	// lambda for the arrow
//	auto arrow = [window](int x, int y, float clr = 0.59) {
//		for (auto i = 5; i >= 2; --i) {
...
//			for (int i = 0; i < items_count; i++) {
//				PushID((void*)(intptr_t)i);
//				const bool item_selected = (*current_item[i] == true);
//				const char* item_text;
//				if (!items_getter(data, current_item[i], &item_text))
//					item_text = "*Unknown item*";
//
//				SetCursorPosX(GetCursorPosX() + 206 - item_size.x - style.ComboOverride.x);
//				styling.ComboOverride.x = 0;
...
bool ImGui::SelectableMeme(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg) {
	...
	// Render
	if (selected) {
		const ImU32 col = GetColorU32((held && hovered) ? ImVec4(config->Misc.menucolor[0], config->Misc.menucolor[1], config->Misc.menucolor[2], 255) : ImVec4(config->Misc.menucolor[0], config->Misc.menucolor[1], config->Misc.menucolor[2], 255));
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
	...
	ImVec2 pos = window->DC.CursorPos;
	pos.y += window->DC.CurrentLineTextBaseOffset;
	ImRect bb(pos + ImVec2(0, 0), pos + size);
	ItemSize(bb);
	...
	if (flags & ImGuiSelectableFlags_Disabled)
		selected = false;
	const ImU32 frame_col = GetColorU32(ImVec4(0.071, 0.067, 0.078, 0)); // MAKRER 1

	// Render
	if (selected)
	{
		RenderFrame(bb_with_spacing.Min, bb_with_spacing.Max, frame_col, false, 0.0f);
	}

	if (flags & ImGuiSelectableFlags_::ImGuiSelectableFlags_DontClosePopups) {
		//window->DrawList->AddRectFilled(bb_with_spacing.Min, bb_with_spacing.Max, frame_col);
	}
	...
	const ImU32 menu_col = GetColorU32(ImVec4(config->Misc.accentcolor[0], config->Misc.accentcolor[1], config->Misc.accentcolor[2], 255)); // MAKRER 1
	const ImU32 menu_gray_col = GetColorU32(ImVec4(config->Misc.accentcolor[0] - 0.3f, config->Misc.accentcolor[1] - 0.3f, config->Misc.accentcolor[2] - 0.3f, 255)); // MAKRER 1

	if (flags & ImGuiSelectableFlags_Disabled) PushStyleColor(ImGuiCol_Text, g.Style.Colors[ImGuiCol_TextDisabled]);
	VRenderText(bb.Min + ImVec2(2, 0), label, NULL, &label_size, selected ? menu_col : hovered ? menu_gray_col : ImColor(255, 255, 255));
	if (flags & ImGuiSelectableFlags_Disabled) PopStyleColor();
...
bool ImGui::MenuItem(const char* label, const char* shortcut, bool selected, bool enabled) {
	...
	if (selected)
		RenderCheckMark(pos + ImVec2(window->MenuColumns.Pos[2] + extra_w + g.FontSize * 0.20f, 0.0f), GetColorU32(enabled ? ImVec4(config->Misc.menucolor[0], config->Misc.menucolor[1], config->Misc.menucolor[2], 255) : ImVec4(config->Misc.menucolor[0], config->Misc.menucolor[1], config->Misc.menucolor[2], 255)));
...
bool ImGui::ColorButton2(const ImVec4& col, bool small_height, bool outline_border)
{
	...
	SetCursorPos(ImVec2(0 + style.ColorOverride.x, GetCursorPosY() + style.ColorOverride.y));
	const ImRect bb(window->DC.CursorPos + ImVec2(286, 3), window->DC.CursorPos + ImVec2(298, 9));
	ItemSize(bb);
	if (!ItemAdd(bb, &id))
		return false;
	...
	styling.ColorOverride = ImVec2(0,0);
	return pressed;
}
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, ImVec2 size) {
	...
	window->DrawList->AddRectFilled(bb.Min - ImVec2(1, 1), bb.Max + ImVec2(1, 1), GetColorU32(ImVec4(config->Misc.menucolor[0], config->Misc.menucolor[1], config->Misc.menucolor[2], 1.f)), 0); // Color button are often in need of some sort of border
...
void ImGui::NextColumn() {
	...
	PushColumnClipRect();
	PushItemWidth(GetColumnWidth() * 0.25f);  // FIXME: Move on columns setup
}
