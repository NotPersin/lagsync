...
public:
	...
	void for_every_entity(const std::function<void(c_cs_player * ent)>& func, int classid)
	{
		for (auto i = 0; i < get_highest_entity_index(); i++)
		{
			const auto entity = get_client_entity(i);
			...
			if (classid != entity->get_client_class()->id)
				continue;

			func(reinterpret_cast<c_cs_player*>(entity));
		}
	}
