...
{
	if (!_config->Visuals.grenade_predict)
		return;
	...
	if (_predicted) {
		for (auto& p : _points) {
			...
			const c_color c = c_color(_config->Colors.grenade_predict[0] * 255, _config->Colors.grenade_predict[1] * 255, _config->Colors.grenade_predict[2] * 255);
